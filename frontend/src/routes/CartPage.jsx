...
function CartPage() {
    ...
    useEffect(() => {
        ...
        const matchedProducts = productsData.filter(product =>
            cartData.some(cartItem => cartItem.productId === product._id)
        );
    ...
    const handleCounterChange = async (e, productId) => {
    ...
    return (
        <>
            ...
            <div className='cart-page'>
                <table>
                    <thead className='cart-page__head'>
                        <tr>
                            <th className='cart-page__head__remove'>Remove</th>
                            <th className='cart-page__head__image'>Image</th>
                            <th className='cart-page__head__product'>Product</th>
                            <th className='cart-page__head__unit-price'>Unit Price</th>
                            <th className='cart-page__head__quantity'>Quantity</th>
                            <th className='cart-page__head__total'>Total</th>
                        </tr>
                    </thead>
                    <tbody className='cart-page__body'>
                        {
                            cartProducts === undefined ? (
                                <tr>
                                    <td className='cart-page__body__remove'>
                                        <SkeletonLoading width='100%' height='50px' />
                                    </td>
                                    <td>
                                        <SkeletonLoading width='100%' height='8rem' />
                                    </td>
                                    <td className='cart-page__body__product'>
                                        <SkeletonLoading width='100%' height='2rem' />
                                    </td>
                                    <td className='cart-page__body__unit-price'>
                                        <SkeletonLoading width='100%' height='2rem' />
                                    </td>
                                    <td className='cart-page__body__quantity'>
                                        <SkeletonLoading width='100%' height='2rem' />
                                    </td>
                                    <td className='cart-page__body__total'>
                                ...
                                <tr key={product._id} className={product.hidden ? "hidden-product" : ""}>
                                    <td className='cart-page__body__remove'>
                                        <button className='cart-page__body__remove__btn' disabled={product.hidden} onClick={() => removeFromCart(product._id)}><i className="fa-solid fa-trash"></i></button>
                                    </td>
                                    <td className='cart-page__body__image'>
                                    ...
                                    <td className='cart-page__body__product'>
                                        ...
                                        {userData?.admin ? <p className='cart-page__body__product__adm-id'>ID: <span>{product._id.toString()}</span> <CopyToClipboard text={product._id}><span><i className="fa-solid fa-copy cart-page__body__product__adm-id__copy"></i></span></CopyToClipboard></p> : <></>}
                                    </td>
                                    <td className='cart-page__body__unit-price'>
                                        ${product.hidden ? '-' : product.price}
                                    </td>
                                    <td className={`cart-page__body__quantity ${product.hidden ? '' : product.countInStock > 0 ? '' : 'out-of-stock'}`}>
                                        <input
                                            type="number"
                                            className='cart-page__body__quantity__input'
                                            value={cartData.find(cartItem => cartItem.productId === product._id).productCount}
                                            min={product.countInStock === 0 ? 0 : 1}
                                            max={product.countInStock}
                                            onChange={(e) => handleCounterChange(e, product._id)}
                                        />
                                    </td>
                                    <td className='cart-page__body__total' >
                                        ${product.price * cartData.find(cartItem => cartItem.productId === product._id).productCount}
                                    </td>
                                </tr>
                            ))
                        }
